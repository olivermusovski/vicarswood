<div class="container-fluid wallpaper-footer d-flex align-items-center">
	<div class="container">
		<div class="row px-3 justify-content-between mt-4">
			<p>
				<a href="{{ url('/privacy-policy') }}" style="color: white;" class="mr-2">{{ __("Privacy and Data Policy") }}</a> |
				<a href="{{ url('/terms-and-conditions') }}" style="color: white;" class="mx-2">{{ __("Sales Terms & Conditions") }}</a> |
				<a href="{{ url('/careers') }}" style="color: white;" class="mx-2">{{ __('Careers') }}</a>
			</p>
			<p style="color: white;">
				Copyright © 2018-2020 Vicarswood All rights reserved.
			</p>
		</div>
		<div class="row px-3 justify-content-end mb-4">
			<i class="fab fa-facebook fa-2x text-white"></i>
			<a href="https://www.instagram.com/vicarswood_las_vegas/"><i class="fab fa-instagram fa-2x text-white ml-3"></i></a>
		</div>
	</div>
</div>

